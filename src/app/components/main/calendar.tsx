...
export class Calendar extends React.Component<CalendarProps, CalendarState> {
  ...
  renderCalendar = () => {
    ...
    for (let i = 0; i < lastWeek; i++) {
      html.push(
        <tr key={Math.random()}>
          {this.renderCalendarDate(i, Utils.convertDateMonthToString(curDate))}
        </tr>
      );
    }
      ...
      if (event.startDate === date) {
        ...
        html[position] = (
          <span
            ...
            onMouseLeave={e => this.handleCalendarHover(e, event, false)}
          >
            {eventTitle}
          </span>
        );
        ...
        html[position] = (
          <span
            ...
            onMouseLeave={e => this.handleCalendarHover(e, event, false)}
          >
            {week === 0 && this.currentDate.getDay() === 0 ? eventTitle : ""}
          </span>
        );
        ...
        <div className="date-wrap main">
          <div className="select-date">
              ...
              {Utils.isEmpty(parsed.week) && (
                <>
                  <span className="date-main">
                    <h2>{parsed.date === null ? "" : `${Number(month)}월`}</h2>
                  </span>
                  <span className={"date-sub year"}>
                    <h4>{parsed.date === null ? "" : `${year}년`}</h4>
                  </span>
                </>
              )}
            ...
            <span
              ...
              onClick={() => this.handleCalendar(preFlag ? "pre" : "pre")}
            >
              &lt;
            </span>
            <span
              className={"current-month"}
              onClick={() => this.handleCalendar("today")}
            >
              오늘
            </span>
            <span
              className={"next "}
              onClick={() => this.handleCalendar("next")}
            >
              &gt;
            </span>
            <div
              className={"create-event-btn"}
              onClick={this.handleGoToTimeTable}
            >
              내 일정
            </div>
          </div>
