...
export class TimeTable extends React.Component<TimeTableProps, TimeTableState> {
  ...
  constructor(props?: any) {
    ...
    if (!isEmpty(location.search) && !isEmpty(parsed.date)) {
  ...
  handleClickCell = (cell: any) => {
    ...
    if (selectCell !== null && !isEmpty(cell)) {
  ...
  renderTimeTable = () => {
    ...
    weekDate.map((date, idx) => {
      html.push(
        <tr
          className={yearMatch === 'true' && date.split(' (')[0] === today ? `date${idx} today active` : `date${idx}`}
          key={idx}
          onClick={() => this.handleClickCell(idx)}
        >
          <td>
            <span className="work-time-date">{date}</span>
            {yearMatch === 'true' && date.split(' (')[0] === today && <span className="today">오늘</span>}
          </td>
          ...
          <div className="select-date">
            <div className="date-label">
              <span className="date-main">
                <h2>{this.currentDate === null ? '' : `${this.currentDate.getMonth() + 1}월`}</h2>
              </span>
              <span className="date-sub">
                <h4>{this.currentDate === null ? '' : `${this.currentDate.getFullYear()}년`}</h4>
                <h4>{weekDate === null ? '' : `${firstDate}  ~ ${lastDate}`}</h4>
              </span>
            </div>
            ...
            <span
              ...
              onClick={() => this.handleTimeTable(preFlag ? 'pre' : 'pre')}
            >
              &lt;
            </span>
            <span className="current-month" onClick={() => this.handleTimeTable('today')}>
              오늘
            </span>

            <span className={'next '} onClick={() => this.handleTimeTable('next')}>
              &gt;
            </span>
            <div className="create-event-btn" onClick={() => this.handleRequestStatus()}>
              요청 현황
            </div>
          </div>
